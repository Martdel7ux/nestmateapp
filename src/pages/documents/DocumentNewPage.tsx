...
export function DocumentNewPage() {
  ...
  const { mutateAsync: upload, isPending } = useUploadDocument(user?.id ?? "");
  ...
  // Auto-detect category and title from filename
  useEffect(() => {
    if (!file) return;
    const nameNoExt = file.name.replace(/\.[^.]+$/, "");
    if (!title) setTitle(nameNoExt);
    setCategory(detectCategory(file.name));
  }, [file]);
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      <AppHeader variant="sub-page" title="Upload Document" />

      <div className="flex-1 overflow-y-auto px-5 pt-4 pb-32 space-y-5">
        {/* File picker */}
        <DocumentUploader value={file} onChange={setFile} error={fileError} />
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g. Apartment lease 2025"
            className={inputCls}
          />
        </div>
