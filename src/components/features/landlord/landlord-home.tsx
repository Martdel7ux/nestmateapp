...
function PropertyCard({
  ...
  const statusLabel = !property.is_approved
    ? "Pending review"
    : property.is_visible
    ? "Active"
    : "Hidden";
...
function AddPropertySheet({ onClose }: { onClose: () => void }) {
  ...
  const [availableTo, setAvailableTo] = useState<"erasmus" | "full_time">("full_time");
        ...
        <div className="space-y-5 px-5 pb-10 pt-2">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Fill in your listing details</p>
            </div>
...
export function LandlordHome() {
  ...
  const myProperties = snapshot.featuredProperties.filter(
    (p) => p.owner_id === profile.id
  );
  const totalViews = myProperties.reduce((sum, p) => sum + (p.views_count ?? 0), 0);
  const activeCount = myProperties.filter((p) => p.is_visible && p.is_approved).length;
  const pendingCount = myProperties.filter((p) => !p.is_approved).length;
  ...
  return (
    <div className="space-y-6 px-5 pt-4 pb-8">
      ...
      <div className="flex items-start justify-between">
        <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.35 }}>
          ...
          <div className="mt-0.5 flex items-center gap-1 text-xs text-muted-foreground">
            <MapPin size={11} />
            <span>{profile.city ? `${profile.city}, Cyprus` : "Cyprus"}</span>
          </div>
        </motion.div>
      ...
      {/* ── Stats grid ── */}
      <div className="grid grid-cols-2 gap-3">
        <StatCard label="Listed properties" value={myProperties.length} icon={Home} accent="bg-primary/10 text-primary" />
        <StatCard label="Total views" value={totalViews} icon={TrendingUp} accent="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" />
        <StatCard label="Active listings" value={activeCount} icon={Eye} accent="bg-sky-500/10 text-sky-600 dark:text-sky-400" />
        <StatCard label="Pending review" value={pendingCount} icon={ShieldCheck} accent="bg-amber-500/10 text-amber-600 dark:text-amber-400" />
      </div>

      {/* ── My properties ── */}
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="font-display text-lg font-bold">My Properties</h2>
          <button
            type="button"
            onClick={() => setAddOpen(true)}
            className="flex items-center gap-1.5 rounded-full bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground shadow-glow transition active:scale-95"
          >
        ...
        {myProperties.length === 0 ? (
          <div className="flex flex-col items-center gap-3 rounded-3xl border-2 border-dashed border-border py-14 text-center">
      ...
      {/* ── Add property sheet ── */}
      <AnimatePresence>
        {addOpen && <AddPropertySheet key="add-sheet" onClose={() => setAddOpen(false)} />}
      </AnimatePresence>
    </div>
  );
}
