    ...
    <div style={{ marginTop: 16 }}>
      ...
      <div style={{ display: "flex", gap: 8, padding: 4, borderRadius: 99, background: "var(--nm-surface2)" }}>
        {([[true, "Upcoming"], [false, "All events"]] as const).map(([v, label]) => (
          <button key={label} type="button" onClick={() => setOnlyUpcoming(v)} className="nm-press" style={{ all: "unset", cursor: "pointer", flex: 1, textAlign: "center", padding: "9px 0", borderRadius: 99, font: "600 13px Inter, sans-serif", background: onlyUpcoming === v ? "var(--nm-surface)" : "transparent", color: onlyUpcoming === v ? "var(--nm-text)" : "var(--nm-muted)", boxShadow: onlyUpcoming === v ? "var(--nm-elev)" : "none" }}>{label}</button>
        ))}
      </div>
          ...
          <div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
              {([["", "Any"], ["in_person", "In person"], ["remote", "Online"], ["hybrid", "Hybrid"]] as const).map(([v, label]) => {
                const on = locType === v;
                return <button key={v || "any"} type="button" onClick={() => setLocType(v)} style={{ all: "unset", cursor: "pointer", padding: "6px 12px", borderRadius: 99, font: "500 12px Inter, sans-serif", background: on ? "var(--nm-accent)" : "var(--nm-surface2)", color: on ? "#fff" : "var(--nm-muted)" }}>{label}</button>;
              })}
            </div>
          </div>
        </div>
...
function EventDetail({ event, onBack }: { event: Opportunity; onBack: () => void }) {
  ...
  return (
    <div style={{ paddingBottom: 96, animation: "nmFade .3s ease-out" }}>
      ...
      <div style={{ marginTop: 16 }}>
        <div style={{ fontSize: 23, fontWeight: 700, letterSpacing: "-.02em", lineHeight: 1.2 }}>{event.title}</div>
      </div>

      {facts.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 14 }}>
          {facts.map((f, i) => (
            <span key={i} style={{ padding: "7px 12px", borderRadius: 99, background: i === 0 ? "var(--nm-soft)" : "var(--nm-surface2)", color: i === 0 ? "var(--nm-accent)" : "var(--nm-muted)", font: "600 12px Inter, sans-serif" }}>{f}</span>
          ))}
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 12, marginTop: 24 }}>
        ...
        {calUrl ? (
          ...
          <div style={{ ...primaryAction, background: "var(--nm-surface2)", color: "var(--nm-muted)", cursor: "default", font: "500 13.5px Inter, sans-serif" }}>Details coming soon</div>
        )}
      </div>
      {calUrl && infoUrl && (
        <a href={infoUrl} target="_blank" rel="noopener noreferrer" className="nm-press" style={{ display: "block", textAlign: "center", marginTop: 12, textDecoration: "none", color: "var(--nm-accent)", font: "600 13.5px Inter, sans-serif" }}>View event page →</a>
      )}
    </div>
  );
}
...
const primaryAction: React.CSSProperties = {
  ...
  display: "flex", alignItems: "center", justifyContent: "center", font: "600 15px Inter, sans-serif",
};
